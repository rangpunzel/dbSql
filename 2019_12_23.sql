...
(p_deptno IN dept.deptno%TYPE,  --들어오는지 나가는지 IN 적어주기.
...
--  );
DECLARE
    -- 부서이름, loc 정보를 저장할수 있는 RECORD TYPE 선언
...
-- java와 다르게 plsql에서는 array 역할을 하는 table type의 인덱스를
-- 숫자 뿐만 아니라, 문자열 형태도 가능하다
-- 그렇기 때문에 index에 대한 타입을 명시한다
-- 일반적으로 array(list) 형태인 경우라면 INDEX BY BINNARY_INTEGER를 주로 사용한다.
-- arr(1).name = 'brown'
-- arr('rercon').name = 'brown'

--dept 테이블의 row를 여러건 저장 할수 있는 dept_tab TABLE TYPE 선언하여
...
-- 복수 ROW의 값을 변수에 저장 : BULK COLLECT INTO
    SELECT *
    BULK COLLECT INTO v_dept    --변수에 값을 저장하기 떄문에 데이터 양에 따라 메모리가 필요.
    FROM dept;
    --CURSOR
...
DECLARE
    ...
    CURSOR dept_cursor IS
        SELECT dname, loc
        FROM dept;
    v_dname dept.dname%TYPE;   --데이터를 한건씩 담아서 출력
    v_loc dept.loc%TYPE;
BEGIN
    -- 커서 오픈
    OPEN dept_cursor;
