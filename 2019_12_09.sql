...
CREATE TABLE dept_test(
        deptno NUMBER(2),
        dname VARCHAR2(14),
        loc VARCHAR2(13), --마지막 컬럼 선어눟 컴마 빼먹지 않기
...
--NOT NULL 제약조건
--해당 컬럼에 NULL값이 들어오는 것을 제한할때 사용
--복합 컬럼과는 거리가 멀다
--기존에 생성한 dept_test 테이블 삭제(drop)
DROP TABLE dept_test;

--컬럼레벨이 아닌, 테이블 레벨의 제약조건 생성
--dname 컬럼에 NULL값이 들어오지 못하도록 NOT NULL
CREATE TABLE dept_test(
        deptno NUMBER(2) PRIMARY KEY,
...
--dname 컬럼에 적용된 UNIQUE제약에 의해 두번째 쿼리는 정상적으로
--실행될 수 없다.
...
--dept_test 테이블 생성 (deptno 컬럼 PRIMARY KET 제약)
...
INSERT INTO dept_test VALUES(99, 'DDIP','daejeon');
COMMIT;
...
--dept_test테이블에 존재하는 deptno로 값을 입력
INSERT INTO emp_test VALUES(9999,'brown',99);
INSERT INTO emp_test VALUES(9998,'sally',99);

--dept_test테이블에 존재하지 않는 deptno로 값을 입력
...
CREATE TABLE emp_test(
   ...
   -- deptno NUMBER(2) REFERENCES dept_test (deptno) 
   deptno NUMBER(2),
   CONSTRAINT KF_dept_test FOREIGN KEY (deptno)
...
--부서정보를 지우려면 지우려고하는 부서번호를 참조하는 직원정보를 
...
INSERT INTO emp_test VALUES (999,'brown',-1,'01');

--check 제약 조건을 위배하지 않으므로 정상 입력(sal, emp_gb)
INSERT INTO emp_test VALUES (9999,'brown',1000,'01');
--cmp_gb check조건에 위배(emp_gb IN ('01','02'))
...
--emp테이블의 데이터를 포함해서 emp_test테이블을 생성
...
--emp테이블의 데이터를 포함해서 emp_test테이블을 컬럼명을 변경하여 생성
...
--emp_test 테이블의 empno컬럼을 PK 제약조건 추가
